...
data Process : Context → Set where
   link      : ∀{Γ A} (p : Γ ≃ [ A ] + [ dual A .force ]) → Process Γ
   fail      : ∀{Γ Δ} (p : Γ ≃ ⊤ , Δ) → Process Γ
   close     : Process  [ 𝟙 ]
   wait      : ∀{Γ Δ} (p : Γ ≃ ⊥ , Δ) → Process Δ → Process Γ
   select    : ∀{A B Γ Δ} (x : Bool) (p : Γ ≃ A ⊕ B , Δ) →
               Process ((if x then A else B) .force ∷ Δ) → Process Γ
   case      : ∀{A B Γ Δ} (p : Γ ≃ A & B , Δ) →
               Process (A .force ∷ Δ) → Process (B .force ∷ Δ) → Process Γ
   join      : ∀{A B Γ Δ} (p : Γ ≃ A ⅋ B , Δ) →
               Process (B .force ∷ A .force ∷ Δ) → Process Γ
   fork      : ∀{A B Γ Δ Γ₁ Γ₂} (p : Γ ≃ A ⊗ B , Δ) (q : Δ ≃ Γ₁ + Γ₂) →
               Process (A .force ∷ Γ₁) → Process (B .force ∷ Γ₂) → Process Γ
   all       : ∀{A Γ Δ} (p : Γ ≃ `∀ A , Δ) ->
               ((B : Type) -> Process (subst (make-subst B) (A .force) .force ∷ Δ)) -> Process Γ
   ex        : ∀{A B Γ Δ} (p : Γ ≃ `∃ A , Δ) ->
               Process (subst (make-subst B) (A .force) .force ∷ Δ) -> Process Γ
   server    : ∀{A Γ Δ} (p : Γ ≃ `! A , Δ) (un : Un Δ) →
               Process (A .force ∷ Δ) → Process Γ
   client    : ∀{A Γ Δ} (p : Γ ≃ `? A , Δ) → Process (A .force ∷ Δ) → Process Γ
   weaken    : ∀{A Γ Δ} (p : Γ ≃ `? A , Δ) → Process Δ → Process Γ
   contract  : ∀{A Γ Δ} (p : Γ ≃ `? A , Δ) → Process (`? A ∷ `? A ∷ Δ) → Process Γ
   cut       : ∀{A Γ Γ₁ Γ₂} (p : Γ ≃ Γ₁ + Γ₂) →
               Process (A ∷ Γ₁) → Process (dual A .force ∷ Γ₂) → Process Γ
