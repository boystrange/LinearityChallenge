...
data _↝_ {Γ} : Process Γ → Process Γ → Set where
  r-link      : ∀{Δ A} {P : Process (dual A .force ∷ Δ)}
                (p : Γ ≃ dual A .force , Δ) →
                cut {A = A} p (link (split-l (split-r split-e))) P ↝ #process (#cons p) P
  r-close     : ∀{P : Process Γ} (p₀ : Γ ≃ [] + Γ) (q₀ : Γ ≃ [] + Γ) →
                cut p₀ close (wait (split-l q₀) P) ↝ P
  r-select-l  : ∀{Γ₁ Γ₂ A B}
                {P : Process (A .force ∷ Γ₁)} {Q : Process (dual (A .force) .force ∷ Γ₂)} {R : Process (dual (B .force) .force ∷ Γ₂)}
                (p : Γ ≃ Γ₁ + Γ₂) (p₀ : Γ₁ ≃ [] + Γ₁) (q₀ : Γ₂ ≃ [] + Γ₂) →
                cut {A = A ⊕ B} p (select true (split-l p₀) P)
                                  (case (split-l q₀) Q R) ↝ cut p P Q
  r-select-r  : ∀{Γ₁ Γ₂ A B}
                {P : Process (B .force ∷ Γ₁)} {Q : Process (dual (A .force) .force ∷ Γ₂)} {R : Process (dual (B .force) .force ∷ Γ₂)}
                (p : Γ ≃ Γ₁ + Γ₂) (p₀ : Γ₁ ≃ [] + Γ₁) (q₀ : Γ₂ ≃ [] + Γ₂) →
                cut {A = A ⊕ B} p (select false (split-l p₀) P)
                                  (case (split-l q₀) Q R) ↝ cut p P R
  r-fork      : ∀{Γ₁ Γ₂ Γ₃ Δ A B}
                {P : Process (A .force ∷ Γ₁)} {Q : Process (B .force ∷ Γ₂)} {R : Process (dual (B .force) .force ∷ dual (A .force) .force ∷ Γ₃)}
                (p : Γ ≃ Δ + Γ₃) (p₀ : Γ₃ ≃ [] + Γ₃) (q : Δ ≃ Γ₁ + Γ₂) (q₀ : Δ ≃ [] + Δ) →
                let _ , p′ , q′ = +-assoc-l p q in
                cut p (fork {A} {B} (split-l q₀) q P Q)
                      (join (split-l p₀) R) ↝ cut q′ P (cut (split-r p′) Q R)
  r-poly       : ∀{A B Γ₁ Γ₂} {P : Process (subst (make-subst B) (A .force) .force ∷ Γ₁)}
                 {F : (C : Type) -> Process (subst (make-subst C) (dual (A .force) .force) .force ∷ Γ₂)}
                 (p : Γ ≃ Γ₁ + Γ₂) (p₀ : Γ₁ ≃ [] + Γ₁) (q₀ : Γ₂ ≃ [] + Γ₂) ->
                 cut p (ex {A} (split-l p₀) P) (all (split-l q₀) F) ↝ cut p P (F B)
  r-client    : ∀{Γ₁ Γ₂ A} {P : Process (A .force ∷ Γ₁)} {Q : Process (dual (A .force) .force ∷ Γ₂)}
                (p : Γ ≃ Γ₁ + Γ₂) (p₀ : Γ₁ ≃ [] + Γ₁) (q₀ : Γ₂ ≃ [] + Γ₂) (un : Un Γ₁) →
                cut p (server {A} (split-l p₀) un P) (client (split-l q₀) Q) ↝ cut p P Q
  r-weaken    : ∀{Γ₁ Γ₂ A} {P : Process (A .force ∷ Γ₁)} {Q : Process Γ₂}
                (p : Γ ≃ Γ₁ + Γ₂) (p₀ : Γ₁ ≃ [] + Γ₁) (q₀ : Γ₂ ≃ [] + Γ₂) (un : Un Γ₁) →
                cut p (server {A} (split-l p₀) un P)
                      (weaken (split-l q₀) Q) ↝ weakening un p Q
  r-contract  : ∀{Γ₁ Γ₂ A}
                {P : Process (A .force ∷ Γ₁)} {Q : Process (`? (dual (A .force)) ∷ `? (dual (A .force)) ∷ Γ₂)}
                (p : Γ ≃ Γ₁ + Γ₂) (p₀ : Γ₁ ≃ [] + Γ₁) (q₀ : Γ₂ ≃ [] + Γ₂) (un : Un Γ₁) →
                cut p (server {A} (split-l p₀) un P)
                      (contract (split-l q₀) Q) ↝
                    contraction un p
                      (cut ++≃+ (server {A} (split-l p₀) un P)
                      (cut (split-r p) (server {A} (split-l p₀) un P) Q))
  r-cut        : ∀{Γ₁ Γ₂ A} {P Q : Process (A ∷ Γ₁)} {R : Process (dual A .force ∷ Γ₂)}
