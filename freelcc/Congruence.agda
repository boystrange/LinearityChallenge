...
data _⊒_ {Σ} {Γ} : Proc Σ Γ → Proc Σ Γ → Set where
  s-comm :
    ∀{A B Γ₁ Γ₂ P Q} (eq : A ~ dual B) (p : Γ ≃ Γ₁ + Γ₂) →
    cut eq (P ⟨ p ⟩ Q) ⊒ cut {A = B} (~symm {!!}) (Q ⟨ +-comm p ⟩ P)
  -- s-link :
  --   ∀{A} (p : Γ ≃ [ A ] + [ dual A ]) →
  ...
  --   cut {A = A} (fail (ch ⟨ > q ⟩ tt) ⟨ p ⟩ P) ⊒ fail (ch ⟨ q′ ⟩ tt)
  -- s-wait :
  --   ∀{Γ₁ Γ₂ Δ A P Q} (p : Γ ≃ Γ₁ + Γ₂) (q : Γ₁ ≃ [ ⊥ ] + Δ) →
  --   let _ , p′ , q′ = +-assoc-l p q in
  --   cut {A = A} (wait (ch ⟨ > q ⟩ P) ⟨ p ⟩ Q) ⊒ wait (ch ⟨ q′ ⟩ cut (P ⟨ p′ ⟩ Q))
  -- s-case :
  --   ∀{A B C Γ₁ Γ₂ Δ P Q R} (p : Γ ≃ Γ₁ + Γ₂) (q : Γ₁ ≃ [ B & C ] + Δ) →
  --   let _ , p′ , q′ = +-assoc-l p q in
  --   cut {A = A} (case (ch ⟨ > q ⟩ (P , Q)) ⟨ p ⟩ R) ⊒
  --   case (ch ⟨ q′ ⟩ (cut (↭proc swap P ⟨ < p′ ⟩ R) ,
  --                    cut (↭proc swap Q ⟨ < p′ ⟩ R)))
  -- s-select-l :
  --   ∀{Γ₁ Γ₂ Δ A B C P Q} (p : Γ ≃ Γ₁ + Γ₂) (q : Γ₁ ≃ [ B ⊕ C ] + Δ) →
  --   let _ , p′ , q′ = +-assoc-l p q in
  --   cut {A = A} (select (ch ⟨ > q ⟩ inj₁ P) ⟨ p ⟩ Q) ⊒
  ...
  --   join (ch ⟨ q′ ⟩ cut (↭proc (↭shift {A} {B ∷ C ∷ []}) P ⟨ < < p′ ⟩ Q))
  -- s-fork-l :
