...
Zero : ∀{Σ} → PreProc Σ [ Nat ]
Zero = select (ch ⟨ < ≫ ⟩ inj₁ (close ch))

Copy : ∀{Σ} → PreProc Σ (dual Nat .force ∷ Nat ∷ [])
Copy = link (ch ⟨ > < • ⟩ ch)

Succ : ∀{Σ} → PreProc Σ (dual Nat .force ∷ Nat ∷ [])
Succ = select (ch ⟨ > < • ⟩ inj₂ (↭proc swap Copy))

One : ∀{Σ} → PreProc Σ [ Nat ]
One = cut (Zero ⟨ ≫ ⟩ Succ)

Add : Proc (dual Nat .force ∷ dual Nat .force ∷ Nat ∷ [])
Add = rec (case {_} {dual Nat} (ch ⟨ < ≫ ⟩ (
                wait (ch ⟨ < ≫ ⟩ Copy) ,
                cut {Nat} (call here (↭concat (> < < •)) ⟨ < < ≫ ⟩ Succ))))
      refl

Two : Proc [ Nat ]
Two = cut (One ⟨ ≫ ⟩ cut (One ⟨ ≫ ⟩ Add))
