...
↭solo : ∀{n} {A : Type n} {Γ Γ′ Δ} → Γ ↭ Δ → Γ ≃ [ A ] + Γ′ → ∃[ Δ′ ] (Δ ≃ [ A ] + Δ′ × Γ′ ↭ Δ′)
↭solo π p with ↭split π p
... | _ , _ , q , π₁ , π₂ rewrite ↭solo-inv π₁ = _ , q , π₂

↭shift : ∀{n} {A : Type n} {Γ Δ} → (Γ ++ A ∷ Δ) ↭ (A ∷ Γ ++ Δ)
↭shift {_} {_} {[]} = refl
↭shift {_} {_} {_ ∷ _} = trans (prep ↭shift) swap

↭concat : ∀{n} {Γ Γ₁ Γ₂ : Context n} → Γ ≃ Γ₁ + Γ₂ → (Γ₁ ++ Γ₂) ↭ Γ
↭concat • = refl
